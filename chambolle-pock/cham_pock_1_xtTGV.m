function res = cham_pock_1_xtTGV_Toep(d, xfm, niter, lambda_x, lambda_t, tol, step, plt_fn)
...
u   =   zeros([xfm.Nd xfm.Nt],'single');
uu  =   zeros([xfm.Nd xfm.Nt],'single');
v   =   zeros([xfm.Nd xfm.Nt 4],'single');
vv  =   zeros([xfm.Nd xfm.Nt 4],'single');
p   =   zeros([xfm.Nd xfm.Nt 4],'single');
q   =   zeros([xfm.Nd xfm.Nt 10],'single');
r2  =   zeros([xfm.Nd xfm.Nt],'single');
d   =   reshape(d, [xfm.Nd xfm.Nt]);
...
while iter < niter && update > tol
    ...
    r2  =   prox(r2 - s*d + s*reshape(mtimes2(xfm, 2*u-uu),[xfm.Nd xfm.Nt]), s, lambda_t); 
